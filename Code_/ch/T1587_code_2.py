...
def develop_malware():
    ...
    print("Malware development complete.")

def develop_code_signing_cert():
    # 產生 self-signed code signing certificate
    subprocess.call(["openssl", "req", "-x509", "-newkey", "rsa:4096", "-keyout", "private.key", "-out", "code_signing_cert.crt", "-days", "365", "-subj", "/CN=My Cert"])

    print("Code signing certificate development completed.")

def develop_ssl_cert():
    # 產生 self-signed SSL/TLS certificate
    subprocess.call(["openssl", "req", "-x509", "-newkey", "rsa:4096", "-keyout", "private.key", "-out", "ssl_cert.crt", "-days", "365", "-subj", "/CN=My Cert"])
